#include <bits/stdc++.h>
#include<windows.h> // for music api
#include<conio.h>
#include<fstream>
#include <Mmsystem.h>// for running multimedia
#pragma comment(lib, "Winmm.lib")//winmm.lib is a import library for multimedia api
using namespace std;
COORD coord = {0, 0};//coord structure to initialize coordinates on the screen
//function to place cursor at given coordinates
void gotoxy (int x, int y)
{
        coord.X = x; coord.Y = y;
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);// SetConsoleCursorPosition sets cursor at specified coordinates at given coords, it takes two parameters GetStdHandle(STD_OUTPUT_HANDLE), it retrieves a handle to the specified standard input or output 
}
...
void textospeech(string w)
{
    ...
    const char* charCommand=command.c_str();// returns a constant char* 
    system(charCommand);//this function return 0 if the command is executed without errors.
}
...
int main()
{
    ...
    while(1)
    {
        ...
        CONSOLE_FONT_INFOEX cfi;.// structure that contains extended information for console font
       ...
       default:
        ...
        if(ch == 0)
        {
            ...
            mciSendString("open \"claps1.mp3\" type mpegvideo alias mp3", NULL, 0, NULL);//sends command string to an mci device ,mci device is a media control interface
